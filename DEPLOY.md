# Cài đặt Striver AIOS trên server / VPS

Striver AIOS là một AI agent cá nhân + Second Brain. "Bộ não" của nó là **Claude Code CLI**
(đăng nhập một lần, không cần API key). Có 3 cách chạy - chọn 1.

> ⚠️ **An toàn:** Striver chạy Claude với toàn quyền trên máy. Khi chạy public (Docker/VPS/Hostinger),
> Striver **tự bật bắt buộc đăng nhập** - mở app ra là màn **tạo tài khoản admin / đăng nhập**, không
> ai điều khiển được khi chưa đặt mật khẩu. (Chạy nội bộ muốn tắt: `AIOS_REQUIRE_LOGIN=0`.)
...
```
https://raw.githubusercontent.com/striverai/striver-aios/main/docker-compose.yml
```
...
> `docker-compose.hostinger.yml` + đặt `DOMAIN_NAME=striver.<hostname-vps>.hstgr.cloud` - xem mục
> **"Link mặc định + HTTPS trên Hostinger"** ở phần HTTPS bên dưới. Compose gốc này chỉ vào được bằng IP:7777.

**3 việc làm 1 lần:**
1. **Để image GHCR ở chế độ Public:** GitHub → repo `striver-os` → **Packages** → `striver-os`
...
2. **Tạo tài khoản admin an toàn** (Claude chạy full quyền nên không để ai cũng tạo được):
   - **Cách A (khuyến nghị):** trong compose của Hostinger, thêm env `AIOS_ADMIN_USER` +
     `AIOS_ADMIN_PASSWORD` → admin tạo sẵn lúc khởi động, mở app ra **đăng nhập luôn**.
...
docker compose run --rm striver claude auth login --claudeai   # ĐĂNG NHẬP CLAUDE 1 LẦN (link + code)
...
Lệnh hằng ngày:
```bash
docker compose logs -f     # xem Striver đang làm gì
docker compose restart     # khởi động lại
docker compose down        # tắt
docker compose build --pull && docker compose up -d   # cập nhật
```

Mọi ghi chú / vault / settings nằm trong Docker volume (`striver-data`, `claude-auth`) →
...
2. Docker Manager → Compose → URL:
   ...
   ```
3. Ô **Environment** (Biến môi trường) đặt **`DOMAIN_NAME=striver.<hostname-vps>.hstgr.cloud`**
   (vd `DOMAIN_NAME=striver.srv1782015.hstgr.cloud`). Muốn tên miền RIÊNG (vd `striveros.com`) thì đặt tên miền đó + trỏ DNS A về IP VPS.
...
> Bấm **Manage → sửa .yaml**, đổi thẳng dòng `Host(...)` thành `Host(\`striver.srv1782015.hstgr.cloud\`)`.
...
2. Mở `http://<ip-vps>:7777` → **⚙ Cài đặt → Tên miền riêng** → nhập `striver.tencuaban.com` → **Lưu**.
3. Bấm **Kiểm tra kết nối** → app hiện đúng bản ghi DNS cần tạo (A: `striver.tencuaban.com → <ip-vps>`).
   Trỏ DNS xong, đợi lan.
4. Mở `https://striver.tencuaban.com` → Caddy **tự xin + gia hạn** chứng chỉ ở lần mở đầu, cookie
   Secure tự bật. Xong.
...
Mở giao diện Striver từ máy khác mà KHÔNG cần mở port / không cần tên miền - như Hermes:

1. **Đặt mật khẩu TRƯỚC (bắt buộc):** mở Striver (qua SSH tunnel ở bước 5) → Dashboard → **Tài khoản** → đặt mật khẩu admin. Striver chạy Claude toàn quyền trên máy → TUYỆT ĐỐI không phơi ra Internet khi chưa có mật khẩu. (Server cũng in cảnh báo nếu chạy public mà chưa đặt.)
2. Bật tunnel: `docker compose --profile tunnel up -d`
3. Lấy URL: `docker compose logs tunnel | grep trycloudflare` → mở `https://<ngẫu-nhiên>.trycloudflare.com` trên trình duyệt bất kỳ, đăng nhập mật khẩu. Giờ xem & thao tác Striver từ xa.

**URL cố định (tên miền riêng, kiểu `*.hstgr.cloud`):** tạo *named tunnel* ở Cloudflare Zero Trust (miễn phí) → lấy token → `TUNNEL_TOKEN=...` vào `.env` → đổi dòng `command` của service `tunnel` sang bản `run --token` (comment sẵn trong `docker-compose.yml`), trỏ tới `http://striver:7777`. Quick tunnel đổi URL mỗi lần restart; named tunnel cho URL ổn định.
...
```
Quản lý dịch vụ: `journalctl -u striver -f` · `sudo systemctl restart striver`

---

## Cách 3 - Windows (máy cá nhân)

Double-click `setup.bat` (chạy hiện cửa sổ) hoặc `start-striver.vbs` (chạy ngầm).
Dừng bằng `stop-striver.bat`. Mở http://localhost:7777
...
|---|---|---|
| `AIOS_HOST` | Địa chỉ nghe. `127.0.0.1` = chỉ máy này; `0.0.0.0` = mọi nơi (Docker tự đặt) | `127.0.0.1` |
| `AIOS_PORT` | Cổng | `7777` |
| `AIOS_STATE_DIR` | Nơi Striver ghi state (settings, sessions, loop config) | `server/` (Docker: `/data/state`) |
...
>   được cấp quyền Docker (socket); app Striver KHÔNG → an toàn. Không muốn thì xoá service đó,
...
git pull && ./.venv/bin/pip install -r requirements.txt && sudo systemctl restart striver   # Native
```
...
"Bộ não" của Striver là Claude Code CLI. Token đăng nhập nằm trong `~/.claude`

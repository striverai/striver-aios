...
Striver AIOS **không phải** một chatbot. Nó là một **lớp điều hành AI** chạy trên máy/VPS của bạn, lấy **CLI của nhà cung cấp AI làm "bộ não"** - **Claude Code CLI** (gói Claude) hoặc **Codex CLI** (gói ChatGPT). Tận dụng chính **gói subscription bạn đang trả** thay vì phải mua thêm API riêng: bộ não đó có đầy đủ khả năng đọc/ghi file, gọi công cụ (MCP), chạy lệnh, dùng skill - rồi Striver gói tất cả vào một **dashboard đẹp, điều khiển bằng giọng nói**, kèm một **Second Brain** (bộ nhớ + wiki) tích luỹ tri thức theo thời gian.

> Triết lý: nhà cung cấp nào có **CLI dạng agent + gói subscription** (Claude Code, Codex, và các CLI ra sau) thì Striver dùng được làm bộ não. Ngoài ra vẫn hỗ trợ chat thuần qua OpenRouter / OpenAI / Anthropic API.

Bạn đấu các **MCP** của riêng mình vào (bán hàng/POS, quảng cáo, lịch, email, ghi chú…) → Striver tự phát hiện và **báo cáo kinh doanh + cuộc sống** bằng số liệu thật, nói chuyện như người.

### Vì sao Striver khác biệt

| | Chatbot thường | **Striver AIOS** |
|---|---|---|
...
> 💡 **Triết lý:** Striver *biên dịch một lần* tri thức từ ghi chú thô → Wiki, rồi *duy trì* nó sống cùng mỗi nguồn mới. Tri thức **tích luỹ**, không tái phát hiện mỗi lần.

---

## ✨ Tính năng nổi bật

- 🎙️ **Trò chuyện bằng giọng nói rảnh tay** - nói, Striver nghe và trả lời bằng giọng (Edge TTS tiếng Việt). Giữ phím Cách để bật mic.
...
- 🧩 **Skills (kiểu Hermes)** - gom nhóm, tìm kiếm, **bật/tắt từng skill**, thêm/sửa/xoá; Striver tự xếp skill mới vào đúng nhóm.
- 🤖 **Agents & Workflows** - tạo trợ lý chuyên biệt + chuỗi tự động (Studio), mỗi bước một agent, có kiểm chứng.
- ♻️ **Tự cải thiện** - Striver tự thức theo lịch làm **một nhiệm vụ cụ thể** (vd: mỗi sáng tổng hợp doanh thu + soạn nháp content) rồi tự kiểm chứng.
...
- 📱 **Telegram bot** - hỏi Striver qua điện thoại, có cả MCP khi dùng engine Claude.
...
> ⚠️ **Quan trọng về bảo mật:** Striver chạy Claude với **toàn quyền** trên máy. Khi chạy public (Docker/VPS/Hostinger), Striver **tự bắt buộc đăng nhập** - mở app ra là màn tạo tài khoản / đăng nhập, không ai điều khiển được khi chưa có mật khẩu.
...
- **Link miễn phí** (không cần mua tên miền): `DOMAIN_NAME=striver.<hostname-vps>.hstgr.cloud`
  (hostname xem ở hPanel → VPS, vd `striver.srv1562015.hstgr.cloud`).
...
**3 việc làm 1 lần:**
1. **Để image GHCR ở chế độ Public:** GitHub → repo → **Packages** → `striver-os` → *Package settings* → Visibility = **Public**.
2. **Tạo tài khoản admin** (chọn 1):
   - *Khuyến nghị:* thêm env `AIOS_ADMIN_USER` + `AIOS_ADMIN_PASSWORD` ở ô Environment → mở app **đăng nhập luôn**.
...
mkdir striver && cd striver
curl -fsSLO https://raw.githubusercontent.com/striverai/striver-aios/main/docker-compose.yml

docker compose run --rm striver claude auth login --claudeai   # đăng nhập Claude 1 lần
docker compose up -d                                          # pull image + chạy
```
Mở `http://<ip-vps>:7777` → màn tạo tài khoản admin (xem MÃ THIẾT LẬP trong `docker compose logs striver`).

### Cách 3 - Cài trực tiếp lên Linux/macOS (không Docker)

```bash
git clone https://github.com/striverai/striver-aios.git striver && cd striver
...
3. Double-click  setup.bat   (chạy hiện cửa sổ)
   hoặc           start-striver.vbs   (chạy ngầm, log ở server\striver.log)
4. Mở http://localhost:7777   ·   Dừng: stop-striver.bat
```
...
Mở Striver → bộ cài đặt sẽ dẫn bạn qua:
...
4. **Đấu MCP** (tuỳ chọn) - vào **MCP**, thêm server (POS, Ads…) bằng URL + key. Striver sẽ báo cáo số liệu thật từ đó.
...
| Mục | Làm gì | Chi tiết |
|---|---|---|
| **Striver** (3D) | Màn chính: trò chuyện (gõ hoặc nói), đồ thị tri thức 3D, panel số liệu trái. | [Trò chuyện & giọng nói](docs/02-tro-chuyen-va-giong-noi.md) · [Đồ thị 3D](docs/03-do-thi-tri-thuc-3d.md) |
...
| **Tự cải thiện** | Bật Striver tự chạy nền làm 1 nhiệm vụ cụ thể theo lịch + nhật ký + LINT Wiki. | [Tự cải thiện](docs/08-tu-cai-thien.md) |
...
| **Kênh** | Bật Telegram bot (hỏi Striver qua điện thoại). | [Kênh Telegram](docs/11-telegram.md) |
...
- **Hỏi số liệu:** *"Doanh thu hôm nay thế nào? So với hôm qua?"* → Striver gọi MCP, trả số thật + đề xuất.
- **Tiêu hoá tri thức (INGEST):** thả file/ghi chú vào → Striver tóm tắt, rút insight, viết vào Wiki, gợi ý task.
...
- **Giọng nói:** bấm mic (hoặc bật rảnh tay) → nói → Striver trả lời bằng giọng. Esc để ngắt.
...
|---|---|---|
| `AIOS_HOST` | Địa chỉ nghe. `127.0.0.1`=chỉ máy này; `0.0.0.0`=public | `127.0.0.1` |
| `AIOS_PORT` | Cổng | `7777` |
| `AIOS_REQUIRE_LOGIN` | `1`/`0` ép bật/tắt bắt buộc đăng nhập (mặc định: bật khi bind public) | *(auto)* |
| `AIOS_ADMIN_USER` / `AIOS_ADMIN_PASSWORD` | Tạo sẵn admin lúc deploy (khỏi cần MÃ THIẾT LẬP) | - |
| `AIOS_STATE_DIR` | Nơi ghi state (settings, sessions, loop) | `server/` (Docker: `/data/state`) |
...
cd striver && ./update.sh          # tự pull image + restart (dữ liệu trong volume KHÔNG mất)
```
...
|---|---|
| Sửa code mà không thấy đổi | Đã đổi `.py`? **Khởi động lại server** (Windows: `stop-striver.bat` → `start-striver.vbs`). Đổi giao diện? **Ctrl+Shift+R**. |
| Port 7777 bị giữ, bản mới không lên | Kill tiến trình cũ TRƯỚC (`stop-striver.bat`, hoặc `taskkill /F /PID <pid>`), rồi start lại. |
| Hostinger không pull được image | Để package GHCR = **Public**; đợi GitHub Action build xong (tab Actions). |
| Mở app báo cần MÃ THIẾT LẬP | App terminal (trong container): `cat /data/state/.setup_token`. Trên host: `docker compose logs striver \| grep "SETUP TOKEN"`. Hoặc đặt env `AIOS_ADMIN_PASSWORD` để khỏi cần mã. |

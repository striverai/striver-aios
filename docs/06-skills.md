...
Một skill trong Javis là một thư mục chứa file `SKILL.md`, đặt tại `.claude/skills/<slug>/SKILL.md` bên trong brain đang chọn. File này có 3 phần đầu (frontmatter) quan trọng:
...
Skill không phải nút bấm thủ công. Nó tự kích hoạt dựa trên trường `description`. Khi bạn gõ hoặc nói một yêu cầu, Javis (chạy trên engine Claude Code) so khớp yêu cầu với `description` của các skill đang bật, thấy khớp thì nạp hướng dẫn tương ứng.
...
Đầu trang có tiêu đề **Skills** kèm dòng trạng thái, ví dụ "3/5 bật · nguồn `.claude/skills`". Con số này cho biết bao nhiêu skill đang bật trên tổng số, và nhắc rằng nguồn skill là thư mục `.claude/skills` của brain hiện tại.

Bên phải tiêu đề là nút **+ Skill** để tạo skill mới.

Nếu brain chưa có skill nào, trang hiện dòng "Brain chưa có skill. Bấm + Skill để tạo (tự lưu vào `.claude/skills` + xếp nhóm)".
...
Khi bạn tắt một skill, Javis chuyển thư mục skill đó vào một chỗ riêng tên là `.disabled` (đường dẫn thành `.claude/skills/.disabled/<slug>`). Đây là cách **tắt thật**: skill nằm trong `.disabled` sẽ không được engine nạp nữa, nên Javis không còn tự dùng nó. Khi bật lại, thư mục được chuyển ngược ra `.claude/skills/<slug>`.
...
Khi lưu, Javis tự sinh **slug** từ tên skill: chuyển thành chữ thường, bỏ dấu tiếng Việt, thay khoảng trắng bằng gạch nối (ví dụ "Viết email" thành `viet-email`). Slug ASCII không dấu giúp Claude Code nạp skill ổn định hơn. Thư mục `.claude/skills/<slug>/SKILL.md` được tạo tự động, bạn không cần tự tạo file.
...
2. Javis hỏi xác nhận: `Xoá skill "<tên>"? Sẽ xoá cả thư mục .claude/skills/<slug>.`
3. Bấm đồng ý để xoá.
...
Bạn không bắt buộc phải điền biểu mẫu. Có thể mở cửa sổ trò chuyện và yêu cầu Javis tạo skill giúp, ví dụ: "Tạo cho tôi một skill viết caption Facebook cho shop mỹ phẩm, kích hoạt khi tôi nhờ viết caption bán hàng." Javis sẽ viết `SKILL.md` và lưu vào `.claude/skills`. Cách trò chuyện xem thêm ở [Trò chuyện & giọng nói](02-tro-chuyen-va-giong-noi.md).
...
Trong trang **Agents**, khi tạo hoặc sửa một agent, bạn thấy phần **Skills** liệt kê các skill có sẵn để tích chọn gán cho agent đó. Agent chỉ liệt kê được skill khi brain đã có skill trong `.claude/skills`; nếu chưa có, phần này hiện ghi chú "Vault chưa có skill trong .claude/skills - vẫn tạo agent được, gán skill sau". Chi tiết ở [Agents & Workflows](07-agents-va-workflows.md).
...
- **Tạo skill nhưng Javis không tự dùng:** kiểm tra `description` đã nêu rõ trigger chưa, và skill có đang **bật** không (thẻ không bị mờ, ô đánh dấu có tích). Ngoài ra skill chỉ chạy khi engine là Claude Code CLI, xem [Models & engine](10-models-va-engine.md).
- **Danh sách trống dù đã tạo skill:** đảm bảo đang xem đúng brain. Nguồn skill là `.claude/skills` của brain đang chọn; đổi brain thì danh sách đổi theo.
...
- [Models & engine](10-models-va-engine.md) - chọn engine Claude Code để skill chạy được.

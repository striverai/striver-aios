...
Trang **Models** là nơi bạn chọn "bộ não" cho Striver: dùng engine nào, model nào để trả lời, đăng nhập vào nhà cung cấp AI, và bật mức suy nghĩ sâu. Đây là trang quyết định Striver thông minh tới đâu, có dùng được công cụ (MCP, skill) hay chỉ trò chuyện thuần.

Nếu bạn mới bắt đầu, xem trước [Bắt đầu & thiết lập lần đầu](01-bat-dau-thiet-lap.md). Khi cần gắn thêm công cụ ngoài cho Striver, xem [MCP & số liệu kinh doanh](09-mcp-va-so-lieu.md).

## Tính năng này là gì

Striver có thể chạy trên nhiều "engine" (nhà cung cấp AI) khác nhau. Bạn chọn 1 cái làm **Main Model** (model chính cho hội thoại), và tùy chọn thêm:
...
Điểm quan trọng nhất cần hiểu: **có 2 cách Striver gọi model**, và chúng khác nhau về khả năng.
...
| Qua **Codex** | OpenAI OAuth (ChatGPT) | Có MCP qua hub (cả kết nối local như Zalo/Webcake) + skill qua router (`striver_use_skill` / đọc file `skills/`) |
...
Nói ngắn gọn: Striver xây trên **CLI dạng agent của nhà cung cấp** - **Claude Code** (gói Claude) và **Codex** (gói ChatGPT). Muốn Striver làm việc thật (đọc/ghi file, gọi công cụ, chạy skill) thì để Main Model ở một trong hai CLI này; cả hai đều tận dụng gói subscription bạn đang trả. Các provider API thẳng (OpenRouter/OpenAI/Anthropic) thiên về trò chuyện, nhanh và tiết kiệm. Agent trong Workflow cũng chọn được model Claude hoặc ChatGPT/Codex - xem [Agents & Workflows](07-agents-va-workflows.md).
...
3. Bấm **Đăng nhập Claude**. Striver hiện một đường link.
4. Mở link đó để đăng nhập tài khoản claude.ai của bạn.
5. Nếu trang hiện **một mã code**, dán mã vào ô rồi bấm **Gửi code**. Một số luồng không cần dán code, Striver tự nhận biết khi đã kết nối.
...
Dùng gói ChatGPT Plus/Pro của bạn thay cho API key. Cách này chạy qua Codex và Striver tự đẩy các MCP của bạn (ví dụ POS bán hàng) sang Codex để ChatGPT cũng gọi được công cụ.

1. Vào **Models**, tìm card **OpenAI OAuth (ChatGPT)**.
2. Bấm **Đăng nhập ChatGPT**.
3. Striver mở một trang xác thực và hiển thị một **mã (user code)**. Ghi nhớ mã này.
4. Mở trang xác thực, nhập mã đó.
5. Striver tự động chờ và kiểm tra. Khi kết nối xong, dòng thông báo hiện **✓ Đã kết nối** và card đổi sang **● Đã kết nối** kèm gói tài khoản.
6. Mã có hiệu lực trong 15 phút. Nếu quá hạn, bấm **Đăng nhập ChatGPT** lại để lấy mã mới.

Muốn ngắt: bấm **Ngắt** trên card này. Nếu ChatGPT đang là Main Model khi bạn ngắt, Striver tự chuyển Main Model về Claude Code để chat không bị gãy.
...
Muốn đổi key sau này: nhập key mới rồi bấm **Đổi key**. Muốn ngắt: bấm **Ngắt** (thao tác này xoá key). Nếu provider đang là Main Model khi bị ngắt, Striver tự chuyển về Claude Code.
...
Danh sách model được nạp động từ chính provider (có nhãn **live**). Nếu không lấy được từ mạng, Striver dùng danh sách dự phòng (nhãn **catalog**). Model bạn chọn được lưu và áp dụng cho phiên chat mới.
...
- **Main Model = Claude Code**: mạnh nhất - đọc/ghi file native, gọi MCP, skill native, loop tự động, session resume. Chế độ khai thác hết sức mạnh Striver AIOS.
- **Main Model = ChatGPT OAuth (Codex)**: gọi được toàn bộ kho Kết nối (hub tự đẩy sang Codex, gồm cả kết nối local như Zalo), có tool file của Codex, và dùng được skill qua router (Striver bơm danh sách skill vào system prompt + tool `striver_use_skill`; Codex chạy cwd=brain nên đọc thẳng `skills/<slug>/SKILL.md`).
- **Main Model = OpenRouter / OpenAI (API) / Anthropic (API)**: từ bản 0.9 cả ba đều gọi được kho Kết nối qua vòng gọi tool, kèm tool đọc/ghi file trong vault và kích hoạt skill (`striver_use_skill`). Khác biệt còn lại so với Claude Code: không có loop nền chạy bằng engine này và không resume session CLI.

Kết luận thực dụng: để Striver "làm việc", giữ Main ở **Claude Code**. Chuyển sang provider API khi bạn chỉ muốn trò chuyện hoặc muốn thử một model cụ thể của hãng khác.
...
- Nếu chỉ muốn Striver nhớ và làm việc trơn tru, đừng đổi Main khỏi Claude Code. Các provider khác dành cho nhu cầu đặc biệt.
...
- Muốn ChatGPT gọi được công cụ bán hàng của bạn: gắn MCP trong trang [MCP & số liệu kinh doanh](09-mcp-va-so-lieu.md) trước, Striver sẽ tự đẩy sang Codex.
...
- **Bấm Ngắt provider đang là Main**: Striver tự chuyển Main về Claude Code để chat không gãy. Đây là hành vi cố ý, không phải lỗi.

...
Trang này gom các trục trặc hay gặp khi dùng Striver AIOS và cách xử lý từng bước. Phần lớn sự cố chỉ cần một trong hai thao tác: khởi động lại server, hoặc tải lại trình duyệt bằng Ctrl+Shift+R. Cuối trang có mục Câu hỏi thường gặp (FAQ) ngắn gọn.

Nếu bạn mới cài Striver lần đầu, xem trước [Bắt đầu & thiết lập lần đầu](01-bat-dau-thiet-lap.md). Nếu bạn đang chỉnh biến môi trường, xem [Cấu hình .env](16-cau-hinh-env.md).
...
1. **Khởi động lại server (khi bạn hoặc bản cập nhật vừa đổi code Python `.py`).**
   - Trên **Windows**: chạy `stop-striver.bat` để tắt, rồi chạy `start-striver.vbs` (chạy ngầm) hoặc `setup.bat` (hiện cửa sổ) để bật lại.
   - Trên **Docker / VPS**: `docker compose restart`.
   - Trên **Linux (systemd)**: `sudo systemctl restart striver`.
...
|---|---|
| Sửa code (hoặc vừa cập nhật) mà **không thấy đổi** | Nếu đổi file `.py`: **khởi động lại server** (Windows: `stop-striver.bat` rồi `start-striver.vbs`; Docker: `docker compose restart`). Nếu chỉ đổi giao diện: nhấn **Ctrl+Shift+R**. |
| **Cổng 7777 bị giữ**, bản mới không lên được | Tắt tiến trình cũ TRƯỚC rồi mới bật lại. Windows: chạy `stop-striver.bat`, hoặc `taskkill /F /PID <pid>` với PID đang giữ cổng. Docker: `docker compose down` rồi `docker compose up -d`. |
| **Hostinger không pull được image** | Đặt package GHCR ở chế độ **Public** (GitHub, repo, mục Packages, chọn `striver-os`, Package settings, Visibility = Public). Sau đó đợi GitHub Action build xong (xem tab Actions của repo) rồi Deploy lại. |
| Mở app **báo cần MÃ THIẾT LẬP** | Lấy mã trong App terminal của container: `cat /data/state/.setup_token`. Nếu chạy trên host: `docker compose logs striver` rồi tìm dòng có `SETUP TOKEN`. Cách khỏi cần mã: đặt sẵn env `AIOS_ADMIN_USER` và `AIOS_ADMIN_PASSWORD` lúc deploy để đăng nhập luôn. |
| **Claude báo chưa đăng nhập** (Striver không trả lời được) | Đăng nhập lại "bộ não" Claude 1 lần. Cách trong app: mở **Models**, ở thẻ Claude Code bấm **Đăng nhập Claude**, mở link, dán code nếu được yêu cầu. Cách bằng lệnh: `claude auth login --claudeai` (Docker: chạy trong App terminal). |
...
Striver gồm hai phần chạy khác nhau, nên cách làm mới cũng khác:

1. **Đổi phần lõi (file Python `.py` trong `server/`)**: server đang chạy vẫn giữ bản cũ trong bộ nhớ. Bạn phải **tắt và bật lại server**:
   - Windows: chạy `stop-striver.bat`, đợi vài giây, rồi chạy `start-striver.vbs`.
   - Docker / VPS: `docker compose restart`.
   - Linux systemd: `sudo systemctl restart striver`.
...
Cổng mặc định của Striver là **7777**. Khi một tiến trình cũ chưa tắt hẳn mà bạn bật bản mới, bản mới sẽ báo lỗi vì cổng đang bận. Xử lý theo thứ tự:

1. Tắt tiến trình cũ trước. Windows: chạy `stop-striver.bat`. Nếu vẫn còn, tìm PID đang giữ cổng rồi `taskkill /F /PID <pid>`. Docker: `docker compose down`.
2. Bật lại. Windows: `start-striver.vbs`. Docker: `docker compose up -d`.

Muốn đổi sang cổng khác (khi 7777 đụng phần mềm khác), đặt biến `AIOS_PORT` trong file `.env`; xem [Cấu hình .env](16-cau-hinh-env.md).
...
1. **Image ở chế độ riêng tư (Private).** Vào GitHub, mở repo, chọn mục **Packages**, chọn `striver-os`, vào **Package settings**, đặt **Visibility = Public**. Có vậy Hostinger mới pull được mà không cần đăng nhập registry.
...
Khi Striver chạy public (Docker/VPS/Hostinger), lần đầu mở app sẽ ra màn tạo tài khoản admin và có thể hỏi **MÃ THIẾT LẬP**. Đây là cơ chế chống người lạ chỉ có URL cũng tạo được tài khoản (vì Claude chạy toàn quyền trên máy). Lấy mã như sau:
...
2. **Trên host (ngoài container)**: chạy `docker compose logs striver` rồi tìm dòng có chữ `SETUP TOKEN`.
3. **Khỏi cần mã**: đặt sẵn admin lúc deploy bằng hai env `AIOS_ADMIN_USER` và `AIOS_ADMIN_PASSWORD` trong compose. Khi đó mở app là đăng nhập luôn, không hỏi mã.

Chi tiết bảo mật và cách đặt mật khẩu xem [Bảo mật & tài khoản](14-bao-mat-tai-khoan.md).

## Claude báo chưa đăng nhập

"Bộ não" của Striver là Claude Code CLI, đăng nhập 1 lần và giữ qua mọi restart/update. Nếu Striver không trả lời hoặc báo chưa đăng nhập:
...
Nếu vào **Tệp tin** mà chỗ danh sách file báo lỗi thay vì lên danh sách, thường do máy chủ đang chạy bản cũ chưa có endpoint Tệp tin (lỗi 404). Bản thân giao diện sẽ nhắc: hãy **khởi động lại server** (Windows: `stop-striver.bat` rồi `start-striver.vbs`) rồi **tải lại trang** bằng Ctrl+Shift+R.
...
1. **Nhật ký hoạt động của Striver khi tự chạy nền**: mở **Tự cải thiện** ở thanh nav trái, kéo xuống mục **Nhật ký gần đây**. Đây là nơi ghi lại các lượt Striver tự thức làm nhiệm vụ. Xem [Tự cải thiện](08-tu-cai-thien.md).
...
3. **Log kỹ thuật của server** (khi cần soi lỗi sâu):
   - Windows chạy ngầm bằng `start-striver.vbs`: log ghi ở `server\striver.log`.
   - Docker / VPS: `docker compose logs striver` (thêm `-f` để xem trực tiếp: `docker compose logs -f`).
   - Linux systemd: `journalctl -u striver -f`.
...
Không, nếu chạy bằng Docker. Mọi ghi chú, vault, settings và cả token đăng nhập Claude nằm trong **Docker volume** (`striver-data`, `claude-auth`), tách khỏi image. Khi bạn cập nhật (bấm **⬆ Cập nhật ngay** trong **Tổng quan**, hoặc chạy `./update.sh` trên VPS), image được thay mới nhưng volume giữ nguyên nên dữ liệu **không mất**. Với bản cài native, dữ liệu nằm trong thư mục brain/vault của repo, cũng không bị `git pull` xoá.
...
Được. Striver hỗ trợ nhiều brain trong cùng một thư mục. Ở dropdown chọn brain trên giao diện, bạn có thể:
...
### Đổi giọng nói của Striver thế nào?
